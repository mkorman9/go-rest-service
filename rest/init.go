...
func initDatasources(config RestBaseConfig) {
	for _, datasource := range config.DataSources {
		source, err := sqlx.Connect(datasource.Type, fmt.Sprintf("%s:%s@tcp(%s:%d)/%s",
			datasource.Username,
			datasource.Password,
			datasource.Host,
			datasource.Port,
		...
		if err != nil {
			panic("Cannot open database connection for name " + datasource.Name + ", " + err.Error())
		}

		GetContext().AddMember("db_" + datasource.Name, source)
	}
}
